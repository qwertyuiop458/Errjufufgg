...
const resolutionReportEl = document.getElementById('resolution-report');
...
async function refreshResolutionReport() {
  if (!currentData) return;
  try {
    const report = await fetch(`/resolution_report/${currentData.session_id}`).then((r) => r.json());
    if (report.error) {
      resolutionReportEl.textContent = report.error;
      return;
    }

    const r = report.resolution || {};
    if (r.width && r.height) {
      const source = r.source === 'manual' ? 'указано вручную' : 'определено автоматически';
      resolutionReportEl.innerHTML = `<p><strong>Разрешение:</strong> ${r.width}×${r.height} (${source})</p>`;
      resWidthEl.value = r.width;
      resHeightEl.value = r.height;
    } else {
      resolutionReportEl.innerHTML = '<p>Разрешение не определено</p>';
    }

    if (report.binary_analysis?.length) {
      const rows = report.binary_analysis
        .slice(0, 6)
        .map((x) => `<li><code>${escapeHtml(x.path)}</code> (${x.size} байт): ${escapeHtml(x.analysis)}</li>`)
        .join('');
      resolutionReportEl.innerHTML += `<ul class="sig-list">${rows}</ul>`;
    }
  } catch (error) {
    resolutionReportEl.innerHTML = `<p class="error">Ошибка отчёта: ${escapeHtml(String(error))}</p>`;
  }
}

applyResolutionBtn.addEventListener('click', async () => {
  if (!currentData) return;
  const width = Number(resWidthEl.value);
  const height = Number(resHeightEl.value);

  if (!width || !height || width < 1 || height < 1) {
    resolutionReportEl.innerHTML = '<p class="error">Введите корректные width/height</p>';
    return;
  }

  try {
    applyResolutionBtn.disabled = true;
    const response = await fetch(`/set_resolution/${currentData.session_id}`, {
      method: 'POST',
    ...
    if (!response.ok || data.error) {
      resolutionReportEl.innerHTML = `<p class="error">${escapeHtml(data.error || 'Не удалось применить разрешение')}</p>`;
      return;
    }
    await refreshResolutionReport();
  } catch (error) {
    resolutionReportEl.innerHTML = `<p class="error">Ошибка применения: ${escapeHtml(String(error))}</p>`;
  } finally {
